...
export async function showSubmissionDetail(submissionId: number): Promise<void> {
    ...
    try {
        const detail = await vscode.window.withProgress(
            {
                location: vscode.ProgressLocation.Notification,
                cancellable: false,
                title: `Loading submission ${submissionId}`,
            },
        ...
        leetCodeChannel.appendLine(`Failed to fetch submission ${submissionId}: ${error}`);
        await promptForOpenOutputChannel("Failed to fetch submission details. Please open the output channel for details.", DialogType.error);
    }
}
